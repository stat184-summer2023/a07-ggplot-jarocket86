...
ggplot(Trips, aes(x = client, y = duration)) +
  geom_boxplot() +
  scale_y_continuous(limits = c(0, quantile(Trips$duration, 0.95))) +
  labs(x = "Client Type", y = "Duration")
```
...
```{r}
ggplot(Trips, aes(x = as.Date(sdate))) +
...
Less people used the bikes as it got colder outside. As well, less people used the bikes on Christimas and Thanksgiving. It appears that there are certain days of the week where people use the bikes less. 


### Day of Week

```{r}
ggplot(Trips, aes(x = format(sdate, "%A"))) +
...
Less people use bikes on the weekend while there is a consistent amount of people who use bikes throughout the week. 
...
```{r}
ggplot(Trips, aes(x = format(sdate, "%H"))) +
  geom_bar(fill = "lightblue", color = "black") +
...
My observations here..... 
People don't use the bikes between midnight and 5 am. People use the bikes the most to get to work at 7-8 and they use them to get home between 4-6. People use bikes consistently between 9-3. After 6 usage of bikes decreases every hour. 
...
```{r}
ggplot(Trips, aes(x = format(sdate, "%M"))) +
  geom_bar(fill = "lightblue", color = "black") +
  labs(x = NULL, y = "Count") +
...
There does not appear to be any relation between minute of the hour and number of bikes used. 
...
### My Code here
grouped <- Trips %>%
  group_by(hour = format(sdate, "%H"), wday = format(sdate, "%w"), client) %>%
  summarise(count = n())

grouped

ggplot(grouped, aes(x = hour, y = count, group = client,color=client)) +
  geom_line() +
...
Casual riders use the bikes the most on the weekends. Registered riders use bikes to get to work and to get home during the week. They use them during the day for the weekends but they are used way less than they are during the week. Casual riders using them during the day during the week. 
...
p <- ggplot() +
  geom_density(data = PairDistances, aes(x = distance, color = "Distance between Stations"), fill = "lightblue", alpha = 0.5) +
  geom_density(data = TripsWithDistances, aes(x = distance, color = "Ride Distance"), fill = NA) +
  labs(x = "Distance", y = "Density") +
  scale_color_manual(values = c("Ride Distance" = "black", "Distance between Stations" = NA)) +
  theme(legend.title = element_blank()) +
  guides(color = guide_legend(reverse = TRUE))+xlim(0,15)
p
```

There seems to be little relation between ride distance and distance between stations. Ride distance probability increases sharply from 0 to 1 miles and then decreases sharply from 1 to 5 miles. Meanwhile, distance between stations probability increases slowly from 1 to 5 miles then decreases slowly from 5 to 15. As probability of finding another station increased the probability of a ride being that distance increased. However, as probability of distance between stations continued to increase ride distance probability decreased to basically 0. 
